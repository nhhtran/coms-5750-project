...
class QuadTree:
    def __init__(self):
        self.bbox = None    # Remove?
        self.node_id = None # Remove?
        self.data = []
        self.nodes = []
...
def build_tree(grid, start, end, max_depth, minx, miny, maxx, maxy):
    ...
    queue.append((make_box(minx, miny, maxx, maxy), max_depth, []))

    while (len(queue) > 0):
        param = queue.popleft()
        bbox, depth, cached_points = param[0], param[1], param[2]
        if ((int(bbox[2]) <= int(bbox[0])) or (int(bbox[3]) <= int(bbox[1]))): continue
        ...
        if (depth == 0 or is_area_minimum(bbox)):
            ...
            is_start_in = is_point_in_bbox(start, bbox)
            is_end_in   = is_point_in_bbox(end,   bbox)
            if (is_start_in != is_end_in):
                quadtree.is_enclosed = True
                return quadtree
            elif (is_start_in):
                queue = deque()
                queue.append((make_box(bbox[0] + 1, bbox[1] + 1, bbox[2] - 1, bbox[3] - 1), depth - 1, []))
                continue
            else:
        ...
        queue.append((make_box(bbox[0], cent[1], cent[0], bbox[3]), depth - 1, bl))
        queue.append((make_box(cent[0], cent[1], bbox[2], bbox[3]), depth - 1, br))
        queue.append((make_box(bbox[0], bbox[1], cent[0], cent[1]), depth - 1, tl))
        queue.append((make_box(cent[0], bbox[1], bbox[2], cent[1]), depth - 1, tr))
...
if __name__ == "__main__":
    ...
    quadtree = build_tree(bgrid, (282, 10), (165, 134), 10, 0, 0, bgrid.shape[0] - 1, bgrid.shape[1] - 1)
    ...
    t = timeit.timeit("build_tree(bgrid, (282, 10), (165, 134), 10, 0, 0, bgrid.shape[0] - 1, bgrid.shape[1] - 1)", globals=globals(), number=15)
    print(t)
